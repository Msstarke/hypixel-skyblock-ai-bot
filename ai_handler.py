...
class AIHandler:
    ...
    ITEM_UPGRADE_MAP = {
        ...
        "maxor helmet":              "MAXOR_HELMET",
        "maxor chestplate":          "MAXOR_CHESTPLATE",
        "maxor leggings":            "MAXOR_LEGGINGS",
        "maxor boots":               "MAXOR_BOOTS",
        "goldor helmet":             "GOLDOR_HELMET",
        "goldor chestplate":         "GOLDOR_CHESTPLATE",
        "goldor leggings":           "GOLDOR_LEGGINGS",
        "goldor boots":              "GOLDOR_BOOTS",
        # ── Dungeons — other ──
    ...
    async def _handle_hypermax_question(self, question: str) -> str | None:
        ...
        # ── Mixed 3/4 set (3 pieces from one set + 1 different piece) ──────────
        # Detected by "3/4" pattern. E.g. "3/4 mineral set with a divan helmet"
        if re.search(r'3\s*/\s*4', q):
            ...
            rep_id   = None
            rep_slot = None  # 0=helmet,1=chest,2=legs,3=boots
                    ...
                    if (iid not in base_piece_ids
                        ...
                        for slot_word, idx in SLOT_IDX.items():
                            if slot_word in iname:
                                ...
                                break
                        if rep_slot is None:
                            rep_slot = 0  # default to helmet slot if no slot word
                        break
            ...
            if base_piece_ids and rep_id is not None and rep_slot is not None:
                ...
                lbin = await self.hypixel.get_lowest_bin()
                # Use the replacement piece to pick reforge (usually more expensive / drives choice)
