    ...
    async def get_response(self, question: str, discord_user_id: int = None) -> str:
        async with self.semaphore:
            ...
            system = (
                ...
                "Keep answers SHORT — 1-3 sentences max unless the user asks for detail. "
                "No bullet lists, no step-by-step guides, no walls of text. Just answer the question directly.\n\n"
                "RULES:\n"
                "- ONLY state facts that are in the knowledge base, live data, or player stats below. "
                "Do NOT invent collection tier rewards, milestone unlocks, specific XP numbers, recipe details, "
                "quest names, or any other specific game data. If it's not in the data provided, say you don't have that info.\n"
                "- General advice (gear progression, what to do next) is OK but keep it brief.\n"
